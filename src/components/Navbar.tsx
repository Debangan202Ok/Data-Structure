...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed inset-x-0 w-screen border-b-[0.5px] md:border-[0.5px] dark:border-[#6a6a6a]  mx-auto flex items-center justify-around md:mt-4 py-3 bg-[#f7f7f7] dark:bg-black tracking-wide shadow-md z-50 md:rounded-full transition-all duration-500 ease-in-out ${onload ? ' md:w-3/5' : 'md:w-1/2'}`}
    >
      <ul className="flex items-center mx-5">
        <li className="font-semibold text-[0.8rem] md:text-[1rem] ">
        <Link href={"/"} className="inline-flex animate-text-gradient bg-gradient-to-t from-[#FFFFFF] via-[#FF6633] to-[#F478e6] bg-[auto_200%] bg-clip-text text-xl text-transparent ">DATA STRUCTURE</Link>
        </li>
