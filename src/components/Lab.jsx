        ...
        <>
            <div className='box-border flex flex-col justify-center items-center w-[100%] pt-[10vh] md:pt-[15vh]'>
                ...
                <div className='box-border w-[90%] flex flex-col rounded-md bg-slate-100 p-5 my-10 shadow-md'>
                    ...
                    <p className='text-justify text-[#6a6a6a] font-mono'>we are using React hooks useState to manage the state of actions, undo stack, and redo stack. The performAction function adds a new action to the actions state and updates the undo and redo stacks accordingly. The undo function removes the last action from the undo stack, adds it to the redo stack, and updates the actions state. Similarly, the redo function removes the last action from the redo stack, adds it to the undo stack, and updates the actions state.</p>
                    <p className='text-justify text-[#6a6a6a] font-mono'>You can use this UndoRedoExample component in your React application to test the undo/redo functionality by performing actions and clicking on the undo and redo buttons. The console log statements provide output to track the state of the undo and redo stacks.</p>
                </div>
            </div>
        </>
    )
